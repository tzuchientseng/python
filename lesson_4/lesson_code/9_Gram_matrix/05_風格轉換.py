...
def gram_matrix(x):
    x = tf.reshape(x, (x.shape[1], x.shape[2], x.shape[3])) # h(高), w(寬), c(顏色)
...
def compute_loss_grads(combination_img, base_img, style_img):
    ...
    with tf.GradientTape() as tape:
        # base_features = model.predict(base_img) # 也可但是會有額外處理(keras API: output層解果)
        base_features = model(base_img) # 取出五個隱藏層的特徵
        loss = tf.zeros(shape=()) # 純量 0
        base_feature = base_features['block5_conv2'] # 第五個隱藏層第二個卷積層
        combination_features = model(combination_img) # 合成圖特徵
        combination_feature = combination_features['block5_conv2']
        """
        1. 計算合成及原始的變異數，依迴歸線的方式計算，也可用 主成分分析 PCA(Principal components analysis), 請自行驗証
        2. 殘差平方總和通常會很大，所以就要再乘上一個極小的數, 比如1e-6(權重)，因此有沒有除以 n, 就沒差了
        tf.reduce_sum()就是計算裏面的總和
        """
        loss = loss + content_weight * tf.reduce_sum( # 離均差平方和。第一次時，因原始跟合成是一樣，所以 loss 為 0
...
def run():
    for i in range(iterations):
        ...
        # matplotlib 是使用 tkinter 套件完成視窗程式，tkinter 是個玩具(會很醜被打槍)，無法商業化，所以就無需理會其執行緒的運作
        # QT5 是商業化的，所以就要注意執行緒的運作 並且linux可以使用
        # 若不會使用執行緒，幾乎任何專案都寫不出來 Thread 非常重要
        if i % 100 == 0:
            ax.clear()
            ax.imshow(img)
            ax.axis('off')
...
if __name__=='__main__':
    ...
    base_img = cv2.imdecode(np.fromfile('sunny.jpg', dtype=np.uint8), cv2.IMREAD_UNCHANGED)
