...
class Age_Edu_Marrige(QueryStrategy):
    def execute(self):
            ...
            if response.status_code == 200:
                ...
                if "responseData" in data:
                    ...
                    for entry in marriage_data:
                        ...
                        # 初始化字典結構
                        if age not in age_edu_totals:
                            age_edu_totals[age] = {}

                        if edu not in age_edu_totals[age]:
                            age_edu_totals[age][edu] = 0

                        # 累加該年齡段和該教育程度的結婚人數
                        age_edu_totals[age][edu] += number_of_marry

                    # 印出各年齡段和教育程度的總結婚人數
                    for age, edu_data in age_edu_totals.items():
                        print(f"年齡段: {age}")
                        for edu, total in edu_data.items():
